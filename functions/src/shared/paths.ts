export const paths = {
  ...
  event: (userId: string, eventId: string): string => `${paths.events(userId)}/${eventId}`,
  ...
  challengePhoto: (userId: string, eventId: string, challengePhotoId: string): string =>
    `${paths.challengePhotos(userId, eventId)}/${challengePhotoId}`,

  // users/${userId}/events/${eventId}/challenge-photos/${challengePhotoId}
  newChallengePhoto: (userId: string, eventId: string, challengePhotoDocId: string, challengePhoto: File): string => {
    const timestamp = Date.now();
    const extension = challengePhoto.name.split(".").pop()?.toLowerCase() || "unknown";
