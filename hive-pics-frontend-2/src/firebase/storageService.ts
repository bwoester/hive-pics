...
export const storageService = {
  ...
  async uploadEventCoverImage (file: File, eventId: string): Promise<string> {
    ...
    const eventCoverImageRef = storageRef(storage, `${eventFolder}/${eventCoverFileName}`);
  ...
  async deleteEventCoverImages (eventId: string): Promise<number> {
    ...
    try {
      ...
      // Filter cover images
      const coverImages = listResult.items
        .filter(item => item.name.startsWith('cover-image'));

      // Delete existing cover images
      const deletionPromises = coverImages
